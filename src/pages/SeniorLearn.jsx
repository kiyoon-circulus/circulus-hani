...
import SeniorLearnByWrite from "@/components/SeniorLearnByWrite";
import SeniorLearnBySpeak from "@/components/SeniorLearnBySpeak";
import SeniorLearnByListen from "@/components/SeniorLearnByListen";
import SeniorLearnByRead from "@/components/SeniorLearnByRead";
...
const SeniorLearn = () => {
  ...
  const { videoRef, submitAnswer, startQuestion } =
  ...
  // 문제 변경 시 집중도 모니터링 시작
  useEffect(() => {
    // console.log("currentIndex = ", currentItemIndex);
    if (item) {
      startQuestion();
    }
  ...
  return (
    <>
      <div className="grid grid-rows-[auto_1fr] md:gap-4 px-6 py-4 rounded-t-3xl w-screen h-screen overflow-hidden">
        ...
        {/* 집중도 상태 표시 NOTE: 현재 표시할 필요는 없어서 가림*/}
        ...
        <div className="flex justify-between items-center">
          <div className="inline-flex items-center">
            <Link
              ...
              to={`/senior/learn/${character}/${chapter}?target=${target}`}
            >
            ...
            <Breadcrumb>
              <BreadcrumbList className="font-bold text-[2.5rem]">
                <BreadcrumbItem>
                  ...
                  >
                    <Link to={`/senior/learn/${character}`}>
                      {TARGETS[target]}
                    </Link>
                  </BreadcrumbLink>
                </BreadcrumbItem>
